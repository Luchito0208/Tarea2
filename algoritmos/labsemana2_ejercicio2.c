...
void mostrarInventario(Jugador J1)
{
	for(int i = 0; i < 100; i++)
	{
		if(J1.inventarioJugador.arrayElemento[i]->id != 0)
		{
			printf("\nElemento con id %d, %d existencias",J1.inventarioJugador.arrayElemento[i]->id,J1.inventarioJugador.arrayElemento[i]->cantidad);
		}
	}
}
...
void eliminarElemento(Jugador J1)
{
	...
	printf("\nQue tipo de objeto desea eliminar?: ");
	scanf("%d", &opcion);
